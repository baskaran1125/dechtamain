...
export class AuthService {
    ...
    async upsertFallbackAdmin(email, password, name) {
        ...
        if (columns.has("name"))
            updateAssignments.push(`name = EXCLUDED.name`);
        if (columns.has("full_name"))
            updateAssignments.push(`full_name = EXCLUDED.full_name`);
        if (columns.has("password"))
            updateAssignments.push(`password = EXCLUDED.password`);
        if (columns.has("password_hash"))
            updateAssignments.push(`password_hash = EXCLUDED.password_hash`);
        if (columns.has("role"))
            updateAssignments.push(`role = EXCLUDED.role`);
        if (columns.has("user_type"))
            updateAssignments.push(`user_type = EXCLUDED.user_type`);
        if (columns.has("verification_status"))
            updateAssignments.push(`verification_status = EXCLUDED.verification_status`);
        if (columns.has("is_approved"))
            updateAssignments.push(`is_approved = EXCLUDED.is_approved`);
        if (columns.has("is_verified"))
            updateAssignments.push(`is_verified = EXCLUDED.is_verified`);
        if (columns.has("profile_complete"))
            updateAssignments.push(`profile_complete = EXCLUDED.profile_complete`);
        const query = `
    ...
    async login(email, password) {
        ...
        if (isFallbackAdminLogin) {
            ...
            try {
                const provisionedAdmin = await this.upsertFallbackAdmin(fallbackAdminEmail, fallbackAdminPassword, fallbackAdminName);
                if (provisionedAdmin) {

...
export class ChatService {
  async getUserInfo(entityType: string, entityId: string) {
    ...
    if (entityType === "user") {
      ...
      return {
        id: entityId,
        type: entityType,
        name: (client as any)?.fullName || client?.name || "Client",
        avatar: null,
      };
      ...
      return {
        id: entityId,
        type: entityType,
        name: (driver as any)?.fullName || driver?.name || "Driver",
        avatar: null,
      };
    }
    return null;
  ...
  async enrichConversations(conversations: any[], forEntityType: string, forEntityId: string) {
    return await Promise.all(
      conversations.map(async (convo) => {
        // Determine the "other" participant
        const isParticipant1 =
          convo.participant1Type === forEntityType &&
  ...
  async createMessageFromSupport(

...
import {
  users, products, orders, catalogItems, drivers, driverDocuments, workerSkills, workerDetails, walletTable, clients, jobs, supportTickets,
  vendorDocuments, workerDocuments, banners, appSettings, locationUpdates, vehiclePricing, manpowerPricing, notifications, notificationReads,
  conversations, messages, typingIndicators,
  type User, type InsertUser,
  type Product, type InsertProduct,
  type Order, type InsertOrder,
  type CatalogItem, type InsertCatalogItem,
  type Driver, type InsertDriver, type DriverDocument,
  type Manpower, type InsertManpower,
  type Client, type InsertClient,
  type Job, type InsertJob,
  type SupportTicket, type InsertSupportTicket,
  type VendorDocument, type ManpowerDocument,
  type Banner, type InsertBanner,
  type LocationUpdate, type InsertLocationUpdate,
  type AppSetting,
  type VehiclePricing, type InsertVehiclePricing,
  type ManpowerPricing, type InsertManpowerPricing,
...
function normalizeUnifiedOrderStatus(status: string | null | undefined): string {
...
function mapOrderStatusForStorage(status: string | null | undefined): string {
  const key = String(status || '').trim().toLowerCase();
  if (!key) return 'Pending';
...
function pickImageCandidate(value: any): string | null {
...
function extractImageUrlFromRow(row: any): string | null {
  return (
    pickImageCandidate(row?.image_url) ||
...
async function tableExistsInPublic(tableName: string): Promise<boolean> {
...
async function ensureAdminOnboardingCompatibilitySchema(): Promise<void> {
  if (await tableExistsInPublic('users')) {
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS verification_status VARCHAR(50)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS rejection_reason TEXT`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_approved BOOLEAN DEFAULT FALSE`).catch(() => ({ rows: [] as any[] }));
  }

  const hasVendors = await tableExistsInPublic('vendors');
  if (hasVendors) {
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS shop_address TEXT`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS shop_latitude NUMERIC(10,8)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS shop_longitude NUMERIC(11,8)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS location_label TEXT`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS location_updated_at TIMESTAMPTZ`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS gst_number VARCHAR(20)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS email VARCHAR(255)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS status VARCHAR(50) DEFAULT 'pending'`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW()`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vendors ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`).catch(() => ({ rows: [] as any[] }));
  }

  if (await tableExistsInPublic('vendor_profiles')) {
    await pool.query(`ALTER TABLE vendor_profiles ALTER COLUMN user_id DROP NOT NULL`).catch(() => ({ rows: [] as any[] }));
  }

  if (await tableExistsInPublic('driver_profiles')) {
    await pool.query(`ALTER TABLE driver_profiles ADD COLUMN IF NOT EXISTS mobile_number VARCHAR(20)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE driver_profiles ADD COLUMN IF NOT EXISTS phone_number VARCHAR(20)`).catch(() => ({ rows: [] as any[] }));
  ...
  if (await tableExistsInPublic('driver_vehicles')) {
    await pool.query(`ALTER TABLE driver_vehicles ADD COLUMN IF NOT EXISTS model_id VARCHAR(100)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE driver_vehicles ADD COLUMN IF NOT EXISTS model_name VARCHAR(100)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE driver_vehicles ADD COLUMN IF NOT EXISTS weight_capacity NUMERIC(10,2)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE driver_vehicles ADD COLUMN IF NOT EXISTS dimensions VARCHAR(100)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE driver_vehicles ADD COLUMN IF NOT EXISTS body_type VARCHAR(50)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE driver_vehicles ADD COLUMN IF NOT EXISTS vehicle_number VARCHAR(50)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE driver_vehicles ADD COLUMN IF NOT EXISTS registration_number VARCHAR(50)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE driver_vehicles ADD COLUMN IF NOT EXISTS license_number VARCHAR(100)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE driver_vehicles ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`).catch(() => ({ rows: [] as any[] }));
  }

  if (await tableExistsInPublic('vehicles')) {
    await pool.query(`ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS model_id VARCHAR(100)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS model_name VARCHAR(100)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS dimensions VARCHAR(100)`).catch(() => ({ rows: [] as any[] }));
    await pool.query(`ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`).catch(() => ({ rows: [] as any[] }));
  }
}

function normalizeStatusToken(value: any): string {
  return String(value || "").trim().toLowerCase();
}

function deriveVendorVerificationStatus(row: any): "pending" | "verified" | "rejected" {
...
export interface IStorage {
  ...
  getProductsByVendor(vendorId: number): Promise<any[]>;
  getApprovedProducts(): Promise<any[]>;
  getPendingProducts(): Promise<any[]>;
  approveProduct(id: number): Promise<Product>;
  rejectProduct(id: number, reason: string): Promise<Product>;
  createProduct(product: InsertProduct & { vendorId: number }): Promise<Product>;

  getCatalogItems(): Promise<CatalogItem[]>;
  createCatalogItem(item: InsertCatalogItem & { vendorId?: number }): Promise<CatalogItem>;
  getStats(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<{ totalUsers: number, totalOrders: number, recentUsers: any[], recentOrders: any[], previousPeriodUsers?: number, previousPeriodOrders?: number }>;

  getOrdersForBuyer(buyerId: number): Promise<any[]>;
  getOrdersForVendor(vendorId: number): Promise<any[]>;
  ...
  getDriverDocuments(driverId: number): Promise<DriverDocument | undefined>;
  upsertDriverDocuments(driverId: number, docs: Partial<DriverDocument>): Promise<DriverDocument>;

  // Manpower
  getManpower(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<any[]>;
  getManpowerById(id: string): Promise<any | undefined>;
  createManpower(worker: InsertManpower, skillData?: { skillName?: string; experience?: string; category?: string }): Promise<any>;
  updateManpowerStatus(id: string, status: string): Promise<any>;

  // Clients
  getClients(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<Client[]>;
  getClient(id: number): Promise<Client | undefined>;
  createClient(client: InsertClient): Promise<Client>;

  // Jobs
  getJobs(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<any[]>;
  getOpsOrders(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<any[]>;
  createJob(job: InsertJob): Promise<Job>;
  updateJobStatus(id: number, status: string): Promise<Job>;
  assignJobDriver(jobId: number, driverId: number): Promise<Job>;
  assignJobWorker(jobId: number, workerId: string): Promise<Job>;
  updateOrderStatus(id: number, status: string): Promise<any>;
  assignOrderDriver(orderId: number, driverId: number): Promise<any>;
  ...
  getConversations(entityType: string, entityId: string): Promise<any[]>;
  getSupportConversations(): Promise<any[]>;
  findOrCreateConversation(p1Type: string, p1Id: string, p2Type: string, p2Id: string, metadata?: any): Promise<Conversation>;
  updateConversationLastMessage(conversationId: number): Promise<void>;
  archiveConversation(id: number): Promise<Conversation>;

  // Messages
  getMessages(conversationId: number, limit?: number, before?: number): Promise<any[]>;
  createMessage(message: InsertMessage): Promise<Message>;
  markMessageAsRead(messageId: number): Promise<Message>;
  markConversationAsRead(conversationId: number, readerType: string, readerId: string): Promise<void>;

  // Onboarding
  getPendingVendors(): Promise<User[]>;
  getAllVendorsWithStatus(): Promise<any[]>;
  getVendorDocuments(vendorId: number): Promise<VendorDocument | undefined>;
  upsertVendorDocuments(vendorId: number, docs: Partial<VendorDocument>): Promise<VendorDocument>;
  verifyVendor(id: number): Promise<User>;
  rejectVendor(id: number, reason: string): Promise<User>;
  getPendingManpower(): Promise<any[]>;
  getAllManpowerWithStatus(): Promise<any[]>;
  getManpowerDocuments(workerId: string): Promise<ManpowerDocument | undefined>;
  upsertManpowerDocuments(workerId: string, docs: Partial<ManpowerDocument>): Promise<ManpowerDocument>;
  verifyManpowerWorker(id: string): Promise<any>;
  rejectManpowerWorker(id: string, reason: string): Promise<any>;
  ...
  getActiveBanners(page?: string): Promise<Banner[]>;
  getBanner(id: number): Promise<Banner | undefined>;
  createBanner(banner: InsertBanner): Promise<Banner>;
  updateBanner(id: number, data: Partial<InsertBanner>): Promise<Banner>;
  deleteBanner(id: number): Promise<void>;

  // Location tracking
  upsertLocation(data: InsertLocationUpdate): Promise<LocationUpdate>;
  getLatestLocations(): Promise<LocationUpdate[]>;
  ...
  getVehiclePricingByType(vehicleType: string): Promise<VehiclePricing | undefined>;
  createVehiclePricing(pricing: InsertVehiclePricing): Promise<VehiclePricing>;
  updateVehiclePricing(id: number, data: Partial<InsertVehiclePricing>): Promise<VehiclePricing>;
  deleteVehiclePricing(id: number): Promise<void>;

  // Manpower Pricing
  getManpowerPricing(): Promise<ManpowerPricing[]>;
  getManpowerPricingByCode(code: string): Promise<ManpowerPricing | undefined>;
  getManpowerPricingByCategory(category: string): Promise<ManpowerPricing[]>;
  createManpowerPricing(pricing: InsertManpowerPricing): Promise<ManpowerPricing>;
  updateManpowerPricing(id: number, data: Partial<InsertManpowerPricing>): Promise<ManpowerPricing>;
  deleteManpowerPricing(id: number): Promise<void>;

  // Notifications
  getNotifications(): Promise<Notification[]>;
  getNotificationById(id: number): Promise<Notification | undefined>;
  getNotificationsForApp(app: string): Promise<Notification[]>;
  createNotification(notification: InsertNotification): Promise<Notification>;
  updateNotification(id: number, data: Partial<InsertNotification>): Promise<Notification>;
  deleteNotification(id: number): Promise<void>;
  markNotificationSent(id: number): Promise<Notification>;
  markNotificationRead(notificationId: number, userId: number): Promise<NotificationRead>;
  getUserNotifications(userId: number, app: string): Promise<Notification[]>;
}

export class DatabaseStorage implements IStorage {
  private mapVendorProfileRow(vp: any, productCount = 0, email = "") {
  ...
  async getApprovedProducts(): Promise<any[]> {
    try {
      const result = await pool.query(`
        SELECT
          p.id,
  ...
  async rejectProduct(id: number, reason: string): Promise<Product> {
    const [updated] = await db.update(products)
  ...
  async createProduct(product: InsertProduct & { vendorId: number }): Promise<Product> {
  ...
  async createCatalogItem(item: InsertCatalogItem & { vendorId?: number }): Promise<CatalogItem> {
  ...
  async getStats(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<{ totalUsers: number, totalOrders: number, recentUsers: any[], recentOrders: any[], previousPeriodUsers?: number, previousPeriodOrders?: number }> {
    try {
      ...
      const usersCountRes = await pool.query(
        `SELECT COUNT(*)::int AS count FROM users ${dateFilterSql}`,
        params
      );
      ...
      let previousPeriodUsers: number | undefined;
      ...
      return {
        ...
        recentUsers: recentUsersRes.rows.map((r: any) => ({
          id: Number(r.id),
          name: r.name,
          email: r.email,
          role: r.role,
  ...
  async getOrdersForBuyer(buyerId: number): Promise<any[]> {
  ...
  async getOrdersForVendor(vendorId: number): Promise<any[]> {
    const results = await db.select({
      id: orders.id,
      quantity: orders.quantity,
      status: orders.status,
  ...
  async createOrder(order: InsertOrder & { buyerId: number }): Promise<Order> {
  ...
  async getDrivers(): Promise<Driver[]> {
    try {
      ...
      const sourceRows = unified.rows && unified.rows.length
        ? unified.rows
        : (await pool.query(`SELECT * FROM drivers ORDER BY id DESC`).catch(() => ({ rows: [] as any[] }))).rows;
  ...
  async getDriverDocuments(driverId: number): Promise<DriverDocument | undefined> {
    ...
    try {
      ...
      if (doc) {
        return {
          ...doc,
          photoUrl: normalizeDocValue((doc as any).photoUrl),
          aadharUrl: normalizeDocValue((doc as any).aadharUrl),
          addressProofUrl: normalizeDocValue((doc as any).addressProofUrl),
          rcBookUrl: normalizeDocValue((doc as any).rcBookUrl),
          licenseUrl: normalizeDocValue((doc as any).licenseUrl),
        } as any;
      }
    } catch {
    ...
    const hasLegacyDriverDocss = await pool.query(
      `SELECT to_regclass('public.driver_documentss') AS table_name`
    ).catch(() => ({ rows: [] as any[] }));
    ...
    for (const row of docsRes.rows || []) {
      ...
      if (!type) continue;
      const preferred =
        normalizeDocValue(row?.front_url) ||
        normalizeDocValue(row?.document_url) ||
        normalizeDocValue(row?.back_url);
      if (preferred) map.set(type, preferred);
    }
  ...
  async upsertDriverDocuments(driverId: number, docs: Partial<DriverDocument>): Promise<DriverDocument> {
    ...
    const [created] = await db.insert(driverDocuments).values({ driverId, ...docs } as any).returning();
    return created;
  }

  // Manpower (worker_skills + worker_details)
  async getManpower(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<any[]> {
    try {
      const unified = await pool.query(`
        SELECT
          wp.id,
  ...
  async createManpower(worker: InsertManpower, skillData?: { skillName?: string; experience?: string; category?: string }): Promise<any> {
  ...
  async updateManpowerStatus(id: string, status: string): Promise<any> {
    const updates: any = {};
    if (status === 'available') { updates.isOnline = true; updates.isFrozen = false; }
    else if (status === 'busy') { updates.isOnline = false; }
    else if (status === 'inactive') { updates.isFrozen = true; updates.isOnline = false; }
  ...
  async getManpowerById(id: string): Promise<any | undefined> {
  ...
  // Clients
  async getClients(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<Client[]> {
    try {
      const unified = await pool.query(`
        SELECT
          cp.id,
  ...
  // Jobs
  async getJobs(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<any[]> {
    try {
      const result = await pool.query(`
        SELECT
          j.id,
  ...
  async getOpsOrders(dateFilter?: { startDate?: Date; endDate?: Date }): Promise<any[]> {
    try {
      ...
      const productIds = Array.from(new Set(filtered.map((o: any) => o.product_id ?? o.productId).filter(Boolean)));
      const buyerIds = Array.from(new Set(filtered.map((o: any) => {
        const v = o.user_id ?? o.buyer_id ?? o.buyerId ?? o.client_id;
        const n = Number(v);
        return Number.isFinite(n) && n > 0 ? n : null;
      }).filter(Boolean)));
      const driverIds = Array.from(new Set(filtered.map((o: any) => o.assigned_driver_id ?? o.assignedDriverId ?? o.driver_id).filter(Boolean)));

      const [productsRes, usersRes, driversRes, vendorProfilesRes] = await Promise.all([
        productIds.length ? pool.query(`SELECT * FROM products WHERE id = ANY($1)`, [productIds]).catch(() => ({ rows: [] as any[] })) : Promise.resolve({ rows: [] as any[] }),
        buyerIds.length ? pool.query(`SELECT * FROM users WHERE id = ANY($1)`, [buyerIds]).catch(() => ({ rows: [] as any[] })) : Promise.resolve({ rows: [] as any[] }),
        driverIds.length ? pool.query(`SELECT * FROM drivers WHERE id = ANY($1)`, [driverIds]).catch(() => ({ rows: [] as any[] })) : Promise.resolve({ rows: [] as any[] }),
        pool.query(`SELECT * FROM vendor_profiles`).catch(() => ({ rows: [] as any[] })),
      ]);

      const productMap = new Map<number, any>();
      for (const p of productsRes.rows as any[]) productMap.set(Number(p.id), p);

      const userMap = new Map<number, any>();
      for (const u of usersRes.rows as any[]) userMap.set(Number(u.id), u);

      const driverMap = new Map<number, any>();
      for (const d of driversRes.rows as any[]) driverMap.set(Number(d.id), d);

      const vpMap = new Map<number, any>();
      for (const vp of vendorProfilesRes.rows as any[]) vpMap.set(Number(vp.id), vp);
  ...
  async assignOrderDriver(orderId: number, driverId: number): Promise<any> {
    const result = await pool.query(
      `UPDATE orders SET assigned_driver_id = $2 WHERE id = $1 RETURNING *`,
      [orderId, driverId]
    );
  ...
  async assignJobDriver(jobId: number, driverId: number): Promise<Job> {
    const [updated] = await db.update(jobs)
  ...
  async assignJobWorker(jobId: number, workerId: string): Promise<Job> {
    const [updated] = await db.update(jobs)
  ...
  async getConversations(entityType: string, entityId: string) {
    const convos = await db
      ...
      .where(
        or(
          and(
            eq(conversations.participant1Type, entityType),
            eq(conversations.participant1Id, entityId)
          ),
          and(
            eq(conversations.participant2Type, entityType),
            eq(conversations.participant2Id, entityId)
          )
        )
      )
  ...
  async findOrCreateConversation(
    p1Type: string, p1Id: string,
    p2Type: string, p2Id: string,
    metadata?: any
  ) {
    ...
    const existing = await db
      ...
      .where(
        or(
          and(
            eq(conversations.participant1Type, p1Type),
            eq(conversations.participant1Id, p1Id),
            eq(conversations.participant2Type, p2Type),
            eq(conversations.participant2Id, p2Id)
          ),
          and(
            eq(conversations.participant1Type, p2Type),
            eq(conversations.participant1Id, p2Id),
            eq(conversations.participant2Type, p1Type),
            eq(conversations.participant2Id, p1Id)
          )
        )
      )
      .limit(1);
  ...
  async updateConversationLastMessage(conversationId: number) {
    await db
  ...
  async getSupportConversations(): Promise<any[]> {
    try {
      ...
      return await Promise.all(
        (convosRes.rows || []).map(async (convo: any) => {
  ...
  async getPendingVendors(): Promise<any[]> {
    try {
      ...
      const [vendorsRes, countsRes] = await Promise.all([
        ...
        `),
        pool.query(`SELECT vendor_id, COUNT(*)::int AS total_products FROM products GROUP BY vendor_id`).catch(() => ({ rows: [] as any[] })),
      ]);
      ...
      return vendorsRes.rows
        .map((vp: any) => this.mapVendorProfileRow(vp, countMap.get(Number(vp.id)) || 0))
        .filter((v: any) => (v.verificationStatus || "pending") === "pending");
    } catch (_error) {
      return [];
    }
  }

  async getAllVendorsWithStatus(): Promise<any[]> {
    try {
      await ensureAdminOnboardingCompatibilitySchema();
      const hasVendorProfiles = await tableExistsInPublic('vendor_profiles');
      const hasVendors = await tableExistsInPublic('vendors');
      ...
      return vendorsRes.rows.map((vp: any) =>
        this.mapVendorProfileRow(vp, countMap.get(Number(vp.id)) || 0)
      );
    } catch (_error) {
      return [];
    }
  ...
  async getVendorDocuments(vendorId: number): Promise<VendorDocument | undefined> {
    try {
    ...
    const profileRes = await pool.query(
      `SELECT user_id FROM vendor_profiles WHERE id = $1 LIMIT 1`,
      [vendorId]
    ).catch(() => ({ rows: [] as any[] }));
  ...
  async upsertVendorDocuments(vendorId: number, docs: Partial<VendorDocument>): Promise<VendorDocument> {
    const profileRes = await pool.query(
      `SELECT user_id FROM vendor_profiles WHERE id = $1 LIMIT 1`,
      [vendorId]
    ).catch(() => ({ rows: [] as any[] }));
    ...
    const docRows: Array<{ type: string; url: string | null | undefined }> = [
    ...
    return (merged || { vendorId, ...docs }) as VendorDocument;
  }

  async verifyVendor(id: number): Promise<User> {
    try {
      await pool.query(
        `UPDATE vendor_profiles SET verification_status = 'verified', rejection_reason = NULL WHERE id = $1`,
        [id]
      );
      await pool.query(
        `UPDATE users u
         SET verification_status = 'verified',
             ...
             status = 'active'
         FROM vendor_profiles vp
         WHERE vp.id = $1 AND vp.user_id = u.id`,
        [id]
      ).catch(() => {});
      await pool.query(
        `UPDATE vendors SET status = 'active' WHERE id = $1`,
        [id]
      ).catch(() => {});
      const rows = await this.getAllVendorsWithStatus();
      return (rows.find((r: any) => Number(r.id) === Number(id)) || null) as any;
    } catch (_error) {
  ...
  async rejectVendor(id: number, reason: string): Promise<User> {
    try {
      await pool.query(
        `UPDATE vendor_profiles SET verification_status = 'rejected', rejection_reason = $2 WHERE id = $1`,
        [id, reason]
      );
      await pool.query(
        `UPDATE users u
  ...
  // Onboarding - Manpower
  async getPendingManpower(): Promise<any[]> {
    try {
      const result = await pool.query(`
        SELECT
          wp.id,
      ...
      return (result.rows || [])
        .filter((r: any) => !r.is_approved && !r.is_frozen)
        .map((r: any) => ({
          id: String(r.id),
          fullName: r.full_name,
          phone: r.phone,
  ...
  async getAllManpowerWithStatus(): Promise<any[]> {
    try {
      const result = await pool.query(`
        SELECT
          wp.id,
  ...
  async getManpowerDocuments(workerId: string): Promise<ManpowerDocument | undefined> {
    const [doc] = await db.select().from(workerDocuments).where(eq(workerDocuments.workerId, workerId));
    return doc;
  }

  async upsertManpowerDocuments(workerId: string, docs: Partial<ManpowerDocument>): Promise<ManpowerDocument> {
  ...
  async verifyManpowerWorker(id: string): Promise<any> {
    try {
      const result = await pool.query(
        `
          UPDATE users u
  ...
  async rejectManpowerWorker(id: string, reason: string): Promise<any> {
    try {
      const result = await pool.query(
        `
          UPDATE users u
  ...
  async getActiveBanners(page?: string): Promise<Banner[]> {
    const now = new Date();
    let active: any[] = [];
    try {
      const result = await pool.query(`
  ...
  async createBanner(banner: InsertBanner): Promise<Banner> {
    try {
      ...
      return this.getBanner(Number(result.rows[0].id)) as any;
    } catch (_error) {
  ...
  async updateBanner(id: number, data: Partial<InsertBanner>): Promise<Banner> {
    try {
      ...
      if (data.subtitle !== undefined) { updates.push(`subtitle = $${i++}`); params.push(data.subtitle); }
      if (data.imageUrl !== undefined) { updates.push(`image_url = $${i++}`); params.push(data.imageUrl); }
      if (data.linkUrl !== undefined) { updates.push(`link_url = $${i++}`); params.push(data.linkUrl); }
      if (data.targetPages !== undefined) { updates.push(`target_pages = $${i++}`); params.push(data.targetPages); }
      if (data.position !== undefined) { updates.push(`position = $${i++}`); params.push(data.position); }
      if (data.active !== undefined) { updates.push(`is_active = $${i++}`); params.push(data.active === "true"); }
      if (data.displayOrder !== undefined) { updates.push(`display_order = $${i++}`); params.push(data.displayOrder); }
      if (data.startDate !== undefined) { updates.push(`start_date = $${i++}`); params.push(data.startDate); }
      if (data.endDate !== undefined) { updates.push(`end_date = $${i++}`); params.push(data.endDate); }

      if (!updates.length) {
        return this.getBanner(id) as any;
      }
  ...
  async upsertLocation(data: InsertLocationUpdate): Promise<LocationUpdate> {
    try {
      // Unified schema stores user_id, not entity_id.
      await pool.query(
        `DELETE FROM location_updates WHERE entity_type = $1 AND user_id = $2`,
        [data.entityType, Number(data.entityId)]
      );
      const result = await pool.query(
        `
    ...
    await db.delete(locationUpdates).where(
      and(
        eq(locationUpdates.entityType, data.entityType),
        eq(locationUpdates.entityId, data.entityId)
      )
    );
  ...
  async getLatestLocations(): Promise<LocationUpdate[]> {
    try {
      const result = await pool.query(`
  ...
  async getAnalytics(): Promise<any> {
    const countQuery = async (table: string): Promise<number> => {
      const res = await pool.query(`SELECT COUNT(*)::int AS count FROM ${table}`).catch(() => ({ rows: [{ count: 0 }] as any[] }));
      return Number(res.rows[0]?.count || 0);
    };

    const groupQuery = async (table: string, field: string): Promise<any[]> => {
      const res = await pool.query(
        `SELECT ${field} AS name, COUNT(*)::int AS value FROM ${table} GROUP BY ${field}`
      ).catch(() => ({ rows: [] as any[] }));
      return (res.rows || []).map((r: any) => ({ name: r.name, value: Number(r.value || 0) }));
    };
    ...
    return {
      ...
      ticketsByStatus,
      ticketsByPriority,
      userGrowth: (userGrowthRes.rows || []).map((r: any) => ({ date: r.date, count: Number(r.count || 0) })),
      jobsOverTime: (jobsOverTimeRes.rows || []).map((r: any) => ({ date: r.date, count: Number(r.count || 0) })),
      driversByStatus,
      workersByApproval: [
  ...
  async getVehiclePricingByType(vehicleType: string): Promise<VehiclePricing | undefined> {
    const [pricing] = await db.select().from(vehiclePricing).where(eq(vehiclePricing.vehicleType, vehicleType));
    return pricing;
  }

  async createVehiclePricing(pricing: InsertVehiclePricing): Promise<VehiclePricing> {
    const [created] = await db.insert(vehiclePricing).values(pricing).returning();
    return created;
  }

  async updateVehiclePricing(id: number, data: Partial<InsertVehiclePricing>): Promise<VehiclePricing> {
  ...
  async deleteVehiclePricing(id: number): Promise<void> {
  ...
  async createManpowerPricing(pricing: InsertManpowerPricing): Promise<ManpowerPricing> {
    const [created] = await db.insert(manpowerPricing).values(pricing).returning();
    return created;
  }

  async updateManpowerPricing(id: number, data: Partial<InsertManpowerPricing>): Promise<ManpowerPricing> {
  ...
  async deleteManpowerPricing(id: number): Promise<void> {
  ...
  private mapLegacyNotificationRow(row: any): Notification {
    return {
      id: Number(row.id),
      title: row.title || "",
      message: row.message || "",
  ...
  async getNotifications(): Promise<Notification[]> {
    try {
      ...
      `);
      return result.rows.map((row: any) => this.mapLegacyNotificationRow(row));
    }
  }
  ...
  async getNotificationsForApp(app: string): Promise<Notification[]> {
    try {
      return await db.select().from(notifications)
        .where(and(
          eq(notifications.status, "sent"),
          sql`(${notifications.targetApp} = 'all' OR ${notifications.targetApp} = ${app})`
        ))
  ...
  async markNotificationSent(id: number): Promise<Notification> {
    try {
      const [updated] = await db.update(notifications)
  ...
  async markNotificationRead(notificationId: number, userId: number): Promise<NotificationRead> {
    try {
  ...
  async getUserNotifications(userId: number, app: string): Promise<Notification[]> {
    try {

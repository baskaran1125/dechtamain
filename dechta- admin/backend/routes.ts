...
const trackingClients = new Set<WebSocket>();

function broadcastLocation(data: any) {
...
export async function registerRoutes(
  ...
  app.post(api.auth.register.path, async (req, res) => {
    try {
      ...
      const registerData = input.role === 'vendor'
        ? { ...input, verificationStatus: 'pending' as const }
        : input;
  ...
  app.get(api.products.vendorList.path, async (req, res) => {
    ...
    try {
      const user = await authService.getMe(userId);
      if (user.role !== "vendor") return res.status(401).json({ message: "Not a vendor" });
  ...
  app.post(api.products.create.path, async (req, res) => {
    try {
      ...
      const user = await authService.getMe(userId);
      if (user.role !== "vendor") return res.status(401).json({ message: "Not a vendor" });
      if (user.verificationStatus !== "verified") return res.status(403).json({ message: "Your account is pending verification. You cannot add products until verified." });
  ...
  app.get(api.ops.stats.path, async (req, res) => {
    ...
    try {
      const user = await authService.getMe(userId);
      if (user.role !== "admin") return res.status(401).json({ message: "Not an admin" });
  ...
  app.post(api.orders.create.path, async (req, res) => {
    try {
      ...
      if (!userId) return res.status(401).json({ message: "Not logged in" });
      const user = await authService.getMe(userId);
      if (user.role !== "buyer") return res.status(401).json({ message: "Only buyers can create orders" });
  ...
  app.patch("/api/ops/drivers/:id/status", async (req, res) => {
    try {
      ...
      try {
        ...
        const refreshed = await driverService.listAll().catch(() => [] as any[]);
        const driver = (refreshed || []).find((d: any) => Number(d?.id) === Number(id)) || { id, status };
        return res.status(200).json(driver);
      }
    } catch (err: any) {
      res.status(500).json({ message: err.message || "Internal server error" });
  ...
  app.post("/api/ops/support/conversations/start", async (req, res) => {
    ...
    try {
      ...
      const convo = await storage.findOrCreateConversation(
        "user", "0",
        withEntityType, String(withEntityId),
        { title: subject || `Chat with ${withEntityType}`, conversationType: "support" }
      );
  ...
  app.get("/api/chat/conversations/:id", async (req, res) => {
    try {
      ...
      const isParticipant =
        (conversation.participant1Type === "user" && conversation.participant1Id === userIdStr) ||
  ...
  app.post("/api/chat/conversations", async (req, res) => {
    try {
      ...
      const conversation = await storage.findOrCreateConversation(
        ...
        withEntityType,
        String(withEntityId),
        { title, conversationType, supportTicketId }
      );
  ...
  app.post(api.ops.createVendor.path, async (req, res) => {
    try {
      ...
      const vendor = await onboardingService.createVendor(sanitized as any);
  ...
  app.put("/api/ops/settings/commission", async (req, res) => {
    ...
    try {
      ...
      if (vc < 0 || vc > 100 || mc < 0 || mc > 100 || dc < 0 || dc > 100) return res.status(400).json({ message: "Commission rate must be between 0 and 100" });
  ...
  app.post(api.ops.createBanner.path, async (req, res) => {
    ...
    try {
      ...
      if (input.startDate) bannerData.startDate = new Date(input.startDate);
      if (input.endDate) bannerData.endDate = new Date(input.endDate);
    ...
    try {
      ...
      const targetPages = Array.isArray(input.targetPages)
        ...
        : typeof input.targetPages === "string"
          ? [input.targetPages]
          : [];
      if (targetPages.length) updateData.targetPages = targetPages.join(',');
  ...
  app.post("/api/vendor/documents", upload.single("document"), async (req, res) => {
    try {
      ...
      if (docType === 'gst') docUpdate.gstUrl = documentUrl;
      else if (docType === 'pan') docUpdate.panUrl = documentUrl;
      else if (docType === 'aadhar') docUpdate.aadharUrl = documentUrl;
      else if (docType === 'business_license') docUpdate.businessLicenseUrl = documentUrl;
  ...
  app.patch("/api/ops/pricing/:id", async (req, res) => {
    ...
    try {
      ...
      if (ratePerKm !== undefined) updateData.ratePerKm = String(ratePerKm);
      if (minKm !== undefined) updateData.minKm = String(minKm);
  ...
  app.patch("/api/ops/manpower-pricing/:id", async (req, res) => {
    ...
    try {
      ...
      if (ratePerHour !== undefined) updateData.ratePerHour = String(ratePerHour);
      if (minHours !== undefined) updateData.minHours = String(minHours);
  ...
  const chatClients = new Map<string, ChatClient>(); // "entityType:entityId" -> ChatClient

  function getChatClientKey(entityType: string, entityId: string) {
    return `${entityType}:${entityId}`;
  }

  function broadcastToConversation(conversationId: number, message: any, excludeKey?: string) {
      ...
      try {
        ...
        if (payload.type === "send_message") {
          ...
          broadcastToConversation(
            conversationId,
            { type: "new_message", data: enriched[0] },
            clientKey
          );
          ...
          broadcastToConversation(
            conversationId,
            { type: "messages_read", data: { conversationId, by: clientKey } },
            clientKey
          );
        }
          ...
          broadcastToConversation(
            conversationId,
            { type: "typing_indicator", data: { conversationId, entityType, entityId, isTyping } },
            clientKey
          );
        }
